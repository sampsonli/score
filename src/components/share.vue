<script>
    ...
    function nativeShare(elementNode, config) {
        ...
        this.share = function (to_app) {
            let title = this.title, url = this.url, desc = this.desc, img = this.img, img_title = this.img_title, from = this.from;
            if (isucBrowser) {
                to_app = to_app === '' ? '' : (platform_os === 'iPhone' ? this.ucAppList[to_app][0] : this.ucAppList[to_app][1]);
                if (to_app === 'QZone') {
                    const B = `mqqapi://share/to_qzone?src_type=web&version=1&file_type=news&req_type=1&image_url=${img}&title=${title}&description=${desc}&url=${url}&app_name=${from}`
        ...
        this.html = function() {
            const position = document.getElementById(this.elementNode);
            const html = '<div class="label">分享到</div>'+
        ...
        this.init = function () {
            const platform_os = this.getPlantform();
        ...
        let share = this;
        let items = document.getElementsByClassName('nativeShare');
    ...
    export default {
        methods: {
            nativeShare (elementNode, config) {
